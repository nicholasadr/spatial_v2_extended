...
disp('Tello Differential Unit Test')
...
model.Xtree{2}(1:6,:) = plux(eye(3), [0, 126, -26]*1E-3); % rotor
model.Xtree{2}(7:12,:) = plux(eye(3), [0, 126, -87]*1E-3); % link
...
model.joint{3}.XtreeInternal = plux(eye(3), [0,0,0]*1E-3); % from first link (gimbal) to second link (thigh)
model.Xtree{3}(1:6,:) = plux(R2, [0, 40, 0]*1E-3); % from predecessor to first rotor
model.Xtree{3}(7:12,:) = plux(R2, [0, -40, 0]*1E-3); % from predecessor to second rotor
model.Xtree{3}(13:18,:) = plux(eye(3), [0, 0, -142.5]*1E-3); % from predecessor to first link (gimbal)
...
model.joint{4}.XtreeInternal = plux(eye(3), [0, 0, -260]*1E-3); % from first link (shank) to second link (foot)
model.Xtree{4}(1:6,:) = plux(R2, [0, 26.55, 0]*1E-3); % from predecessor to first rotor
model.Xtree{4}(7:12,:) = plux(R2, [0, -26.55, 0]*1E-3); % from predecessor to second rotor
model.Xtree{4}(13:18,:) = plux(eye(3), [0, 0, -226.8]*1E-3); % from predecessor to first link (shank)
...
model.Xtree{5}(1:6,:) = plux(eye(3), [0, -126, -26]*1E-3); % rotor
model.Xtree{5}(7:12,:) = plux(eye(3), [0, -126, -87]*1E-3); % link
...
model.joint{6}.XtreeInternal = plux(eye(3), [0,0,0]*1E-3); % from first link (gimbal) to second link (thigh)
model.Xtree{6}(1:6,:) = plux(R2, [0, 40, 0]*1E-3); % from predecessor to first rotor
model.Xtree{6}(7:12,:) = plux(R2, [0, -40, 0]*1E-3); % from predecessor to second rotor
model.Xtree{6}(13:18,:) = plux(eye(3), [0, 0, -142.5]*1E-3); % from predecessor to first link (gimbal)
...
model.joint{7}.XtreeInternal = plux(eye(3), [0, 0, -260]*1E-3); % from first link (shank) to second link (foot)
model.Xtree{7}(1:6,:) = plux(R2, [0, 26.55, 0]*1E-3); % from predecessor to first rotor
model.Xtree{7}(7:12,:) = plux(R2, [0, -26.55, 0]*1E-3); % from predecessor to second rotor
model.Xtree{7}(13:18,:) = plux(eye(3), [0, 0, -226.8]*1E-3); % from predecessor to first link (shank)
...
q   = vertcat([1 0 0 0 0 0 0]', rand(model.NQ-7,1));
q   = normalizeConfVec(model, q);
q_dot  = vertcat([0 0 0 0 0 0]', rand(model.NV-6,1));
q_ddot = vertcat([0 0 0 0 0 0]', rand(model.NV-6,1));

% Sanity check for full config
q = [1. 0. 0. 0. 0. 0. 0. 0.1078 0.9063 0.8797 0.8178 0.2607 0.5944 0.0225 0.4253 0.3127 0.1615]';
q_dot = [0. 0. 0. 0. 0. 0. 0.1788 0.4229 0.0942 0.5985 0.4709 0.6959 0.6999 0.6385 0.0336 0.0688]';
q_ddot = [0. 0. 0. 0. 0. 0. 0.3196 0.5309 0.6544 0.4076 0.8200 0.7184 0.9686 0.5313 0.3251 0.1056]';
