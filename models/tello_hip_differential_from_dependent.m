function robot = tello_hip_differential_from_dependent(qd, qd_dot, qd_ddot)
...
% obtain independent coordinate, velocity, accelerations
name = 'IK_dependent_state_to_';
y_fn = external(strcat(name,'y'), './models/casadi_fn/y_gen.so');
y = full(y_fn(qd));
y_dot_fn = external(strcat(name,'y_dot'), './models/casadi_fn/y_dot_gen.so');
y_dot = full(y_dot_fn(qd,qd_dot));
y_ddot_fn = external(strcat(name,'y_ddot'), './models/casadi_fn/y_ddot_gen.so');
y_ddot = full(y_ddot_fn(qd,qd_dot,qd_ddot));
assert(~isnan(y_dot(1)) && ~isnan(y_dot(2)))
assert(~isnan(y_ddot(1)) && ~isnan(y_ddot(2)))

% state position, velocity, acceleration
q = [y(1) y(2) qd(1) qd(2)]';
q_dot = [y_dot(1) y_dot(2) qd_dot(1) qd_dot(2)]';
q_ddot = [y_ddot(1) y_ddot(2) qd_ddot(1) qd_ddot(2)]';
