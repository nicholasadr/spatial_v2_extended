...
v = -inv_skew(E);				% v is 2*q0 * [q1;q2;q3]

%casadi_zero = SX.zeros(1,1);
%sym_logic_comparison = tr > casadi_zero;

q = SX.sym('quat',4);
aa = SX.sym('aa',4);
bb = SX.sym('bb',4);

q = if_else(tr>0, Etoq_if(tr,v), Etoq_else(E,tr,v));
%q = if_else(tr>0, aa, bb);
...
function q = Etoq_if(tr,v)
  import casadi.*
  q = SX.sym('q_if',4);
  q = [ (tr+1)/2; v ];

function q = Etoq_else(E,tr,v)
  import casadi.*
  q = SX.sym('q_else',4);
  E = E - (tr-1)/2 * eye(3);
  E = E + E';
  q = if_else(E(1,1)>=E(2,2) & E(1,1) >= E(3,3), Etoq_else_if(E,v), Etoq_else_else(E,v));
  q = if_else(E(2,2)>=E(3,3), Etoq_else_elif(E,v), Etoq_else_else(E,v));
  q = if_else(q(1)<0, -q, q);
  %{
  if E(1,1) >= E(2,2) && E(1,1) >= E(3,3)
    q = [ 2*v(1); E(:,1) ];
...
function q = Etoq_else_if(E,v)
  import casadi.*
  q = SX.sym('q_else_if',4);
  q = [2*v(1); E(:,1) ];

function q = Etoq_else_elif(E,v)
  import casadi.*
  q = SX.sym('q_else_elif',4);
  q = [ 2*v(2); E(:,2) ];
  
function q = Etoq_else_else(E,v)
  import casadi.*
  q = SX.sym('q_else_else',4);
  q = [ 2*v(3); E(:,3) ];